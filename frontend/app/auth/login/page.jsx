...
const LoginPage = () => {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      if (res.data.user.role === 'DRIVER') router.push('/driver/dashboard');
          ...
          {step === 1 ? (
            <>
              ...
              <p className="text-center text-sm text-gray-500">
                New here?{' '}
                <Link href="/auth/register" className="text-orange-400 font-semibold hover:text-orange-300 transition-colors">
                  Create an account
                </Link>
              </p>
            </>
            ...
            <>
              <div className="text-center space-y-1">
                ...
                <p className="text-sm text-gray-400">
                  OTP sent to <span className="text-orange-400 font-semibold">{email}</span>
                </p>
              </div>

              <form onSubmit={handleVerifyOTP} className="space-y-4">
                <div className="space-y-1.5">
                  <label htmlFor="otp" className="text-xs font-semibold text-gray-400 uppercase tracking-wide">
                    Enter OTP
                  </label>
                  <input
                    id="otp"
